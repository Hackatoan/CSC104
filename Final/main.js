...
function renderSidebarPlaylists() {
  ...
  if (playlists.length === 0) {
    ...
    playlists.forEach((pl) => {
      ...
      btn.onclick = () => {
        location.hash = `#playlist/${encodeURIComponent(pl.name)}`;
      };
...
contactBtn.onclick = () => (location.hash = "#contact");
...
function renderDefaultContent() {
  document.title = "Simple Music App - Home";
  currentPlaylistView = null;
  searchInput.value = "";
...
function renderPlaylistContent(playlistName) {
  document.title = `Playlist: ${playlistName}`;
  const playlists = getPlaylists();
  ...
  let html = `
    <h2 class="playlist-header">
      ${playlist.name}
      <button id="delete-playlist-btn" title="Delete Playlist">&#128465;</button>
    </h2>
  `;
...
function renderAboutContent() {
  document.title = "Simple Music App - About";
  searchInput.value = "";
  mainContent.innerHTML = `
    <section id="about-section">
      ...
      <p>All songs featured in this application are sourced from <a href="https://ncs.io/" target="_blank">NCS.io</a> (NoCopyrightSounds), providing royalty-free music for creators.</p>
      <p>Album cover images are dynamically generated and provided by <a href="https://picsum.photos/" target="_blank">Picsum.photos</a>.</p>
      <p>Favicon is generated by Gemini and ran through <a href="https://favicon.io/favicon-converter/" target="_blank">favicon.io</a>.</p>
...
function router() {
  ...
  if (path.startsWith("#playlist/")) {
    const playlistName = decodeURIComponent(path.substring(10));
...
function renderContactContent() {
  document.title = "Simple Music App - Contact";
  searchInput.value = "";
  mainContent.innerHTML = `
...
fetch("../../music/songs.json")
  ...
  .then((songs) => {
    ...
    updateShuffleButton();
    updateLoopButton();
    router(); // Render the initial page based on the URL
  });
